...
def get_new_tgz_file():
    for file in os.listdir("." if not os.path.isdir("apps") else "apps"):
        if file.endswith(".tgz"):
            return os.path.join("." if not os.path.isdir("apps") else "apps", file)
    return None
...
def promote_to_ready_for_prod(app_name, tgz_path, tmp_path):
    ...
    app_dest = Path(DEST_DIR) / f"{app_name}.tgz"
    shutil.copy(tgz_path, app_dest)
    ...
    subprocess.run(["git", "add", DEST_DIR, EXTRACTED_DIR], check=True)
